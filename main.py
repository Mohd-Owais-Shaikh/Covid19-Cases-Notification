...
def send_via_email():
    ...
    count_list = dataset.get_count_list(det.states_and_cities, det.state_url, det.district_url)

    for entry in det.email_list:
        s = smtplib.SMTP('smtp.gmail.com', 587)
        s.starttls()
        s.login(cred.login_email, cred.login_pass)
        message = dataset.form_message_for_email(count_list, entry.get('states'), entry.get('cities'))
